...
export default function Sidebar() {
  ...
  return (
    <aside className="hidden md:flex flex-col w-72 shrink-0 border-r border-[#332d24] h-screen sticky top-0 bg-[#12130f]/95 backdrop-blur-xl">
    ...
    <li className="relative group">
      <Link
        ...
        className={cn(
          ...
          isActive
            ? "bg-[#b56f2a]/10 text-[#f0bd68] font-medium shadow-sm ring-1 ring-[#b56f2a]/25"
        ...
        {Icon && (
          <Icon
            className={cn(
              ...
              isActive ? "text-[#e1ad5a]" : "text-neutral-400 group-hover:text-neutral-300 dark:text-neutral-500 dark:group-hover:text-neutral-300"
            )}
          />
        )}
      ...
      {link.description && (
        <div className="absolute left-full top-1/2 -translate-y-1/2 ml-2 w-64 p-3 bg-neutral-900 dark:bg-neutral-800 text-white text-xs rounded-lg shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 pointer-events-none translate-x-2 group-hover:translate-x-0 border border-neutral-700/50">
          <div className="font-semibold mb-1 text-[#f0bd68]">{link.label}</div>
